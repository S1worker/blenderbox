...
class BuildPage extends Controller
{
    ...
    public function newsEvents($args = [])
    {
        ...
        if( wp_doing_ajax() ) :
             ...
             * Month | acf field
             *
             * Если в запрос приходит фильтр по месяцам, тогда мы обрабатываем дату
             * и включаем ее в выборку записей
             *
             * @array
             */
            if( sanitize_text_field($_POST['month']) && !empty(sanitize_text_field($_POST['month'])) ) :
                ...
                /**
                 * Здесь мы проверяем что к пользователь нажал следующий или предыдущий месяц
                 *
                 */
                ...
                /**
                 *
                 * Добавляем к запросу выборку
                 * по месяцу и соответственно
                 * по году
                 *
                 */
        ...
        if( wp_doing_ajax() ) :
            ...
            if( 'post' == $type ) :
                ...
                if( sanitize_text_field($_POST['place']) && !empty(sanitize_text_field($_POST['place'])) ) :
                    ...
                    $places     = explode( ',', sanitize_text_field($_POST['place']) );
